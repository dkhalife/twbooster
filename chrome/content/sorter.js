/**
 * TWB Usage : 
 * <table class="example altstripe sort01 table-autostripe table-autosort:2 table-stripeclass:alternate2">
 *	<thead>
 *		<tr>
 *			<th class="table-sortable:numeric table-sortable table-sorted-desc" title="Click to sort">Index</th>
 *			<th class="table-sortable:numeric table-sortable" title="Click to sort">Numeric</th>
 *			<th class="table-sortable:default table-sortable" title="Click to sort">Text</th>
 *			<th class="table-sortable:currency table-sortable" title="Click to sort">Currency</th>
 *			<th class="table-sortable:date table-sortable" title="Click to sort">Date</th>
 *			<th class="table-sortable:default table-sortable" title="Click to sort">Checkbox</th>
 *		</tr>
 *	</thead>
 *	<tbody>
*/
...
//table-autopage:5 table-stripeclass:alternate

function TWB_RCBB(){
	cts="";
	rows=document.getElementById("rctable").childNodes[0].childNodes[1].childNodes;
	for(u=0; u<rows.length; u++){
		cells=rows[u].childNodes;
		for(z=0; z<cells.length; z++){
			X=cells[z].textContent;
			if(X==""){
				X="N/A ";
			}
			switch(z){
				...
				case 2:
					// Village
					coords=X.match(/\d+\|\d+/)[0];
					cts+="[village]"+coords+"[/village] ";
				break;
	...
	// Display Text area
	document.getElementById("rcbbcode").parentNode.style.display="";
	// Add BBcodes 
	document.getElementById("rcbbcode").value=cts;
}

function T_TWB_RCBB(e){
	if(!e.target.checked){
		...
		// Button is now normal
		e.target.checked=false;
		// Show table
		document.getElementById("rctable").style.display="";
		// Hide Text area
		document.getElementById("rcbbcode").parentNode.style.display="none";
	}
}

window.addEventListener("load",function(){
	// This is for BB in RC
	if(document.getElementById("rcbbcode")){
		document.getElementById("RCBB").addEventListener("click",T_TWB_RCBB,false);
	}
},false);